// 主菜单：游戏选择 + 玩法说明 + 摄像头预说明
export class Menu {
  constructor(ctx) { this.ctx = ctx; this.root = null; }
  ...
  async mount(root) {
    ...
    wrap.style.cssText = 'position:absolute;inset:0;display:flex;align-items:center;justify-content:center;padding:20px;overflow:auto;background:radial-gradient(ellipse at top, #1a1f3a, #05060f 70%);';
    ...
    wrap.querySelector('#goFruit').onclick = () => this.ctx.router.go('fruit');
    wrap.querySelector('#goHero').onclick = () => this.ctx.router.go('hero');
    const audioBtn = wrap.querySelector('#toggleAudio');
    audioBtn.onclick = () => {
      const m = !this.ctx.audio.muted;
      this.ctx.audio.setMuted(m);
      audioBtn.textContent = m ? '🔇 音效：关' : '🔊 音效：开';
    };
  }
